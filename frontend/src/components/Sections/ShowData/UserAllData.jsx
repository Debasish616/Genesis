...
function UserAllData(props) {
  ...
  async function showDecryptedData(n) {
    try {
      const _fullURL = await getFileUrlFromIpfsHash(dataArray[n].dataHash);
      const _res = await fetch(_fullURL);
      const encryptedFile = await _res.blob();
      const decryptedFile = await decryptFile(encryptedFile, dataArray[n].decryptKey);
      if (props.type === DataTypeEnum.CREDENTIALS) {
        const formData = JSON.parse(await unZipAndGetData(decryptedFile));
  ...
  async function handleDecryptData(n) {
    if (!address) return
